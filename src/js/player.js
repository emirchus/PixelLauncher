...
var alpha = 0;
var mouseRotation = 0;
camera = new THREE.PerspectiveCamera(75, 500 / 400, 1, 10000);
...
function reloadSkinTexture(file) {
    img.onload = function () {
...
var controller = document.getElementById("modelcontroller")
var pos1 = 0, pos3 = 0;
function updateX(f) {
    f = f || window.event;
    pos3 = f.clientX;
    window.onmouseup = cancelAll
    window.onmousemove = calculateAlpha;
}
function cancelAll() {
    window.onmouseup = null;
    window.onmousemove = null
}
function calculateAlpha(e) {  
    pos1 = pos3 - e.clientX;
    //pos3 = e.clientX
    mouseRotation = (model.offsetLeft / pos1);
}
function Animate() {
    requestAnimationFrame(Animate);

    controller.onmousedown = updateX;


    camera.rotation.y = mouseRotation;

    camera.position.z = radius * Math.cos(mouseRotation);
    camera.position.x = radius * Math.sin(mouseRotation);
    alpha += Math.PI / 320;
    //Leg Swing
